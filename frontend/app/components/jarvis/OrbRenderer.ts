import { ORB_STATE_HUES, type OrbState } from "./jarvisTypes";
...
export class OrbRenderer {
  ...
  setState(state: OrbState) {
    if (state === "success" && this.state !== "success") this.flashUntil = this.t + 0.4;
    this.state = state;
  }

  setLevel(level: number) { this.level = Math.max(0, Math.min(1, level)); }
  ...
  private frame(dt: number) {
    ...
    this.smoothLevel = this.level > this.smoothLevel
    ...
    // outer ambient glow
    const g1 = ctx.createRadialGradient(cx, cy, coreR * 0.4, cx, cy, R * 3.1);
    g1.addColorStop(0, `hsla(${hue} 95% 60% / ${0.16 * intensity + lvl * 0.1})`);
    g1.addColorStop(0.5, `hsla(${hue} 90% 55% / ${0.05 * intensity})`);
    ...
    if (this.current.waveAlpha > 0.02) {
      ...
      for (let i = 0; i < bars; i++) {
        ...
        const n = Math.sin(this.t * 5 + this.wavePhases[i]) * 0.4 + Math.sin(this.t * 11 + this.wavePhases[(i * 7) % 128]) * 0.25;
    ...
    // core sphere
    const g2 = ctx.createRadialGradient(cx - coreR * 0.3, cy - coreR * 0.35, coreR * 0.1, cx, cy, coreR);
    g2.addColorStop(0, `hsla(${hue} 100% 88% / ${0.95 * intensity + 0.05})`);
    g2.addColorStop(0.35, `hsla(${hue} 95% 62% / ${0.55 * intensity + 0.1})`);
    g2.addColorStop(0.8, `hsla(${hue} 90% 42% / ${0.28 * intensity})`);
    g2.addColorStop(1, `hsla(${hue} 90% 35% / 0.05)`);
    ctx.beginPath();
    ...
    for (let i = 0; i < filaments; i++) {
      ...
      ctx.beginPath();
      ctx.ellipse(cx, cy, coreR * (0.55 + 0.12 * Math.sin(fPhase * 0.7)), coreR * 0.3, fPhase, 0, Math.PI * 2);
